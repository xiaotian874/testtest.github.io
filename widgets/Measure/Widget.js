...
///////////////////////////////////////////////////////////////////////////
//
//  05/04/2017 - added functionality to display an interactive polyline total when drawing two or more segments.
    ...
    function(declare, _WidgetsInTemplateMixin, BaseWidget, GraphicsLayer, Edit, Graphic, Extent, Point,
        ...
        return declare([BaseWidget, _WidgetsInTemplateMixin], {
            ...
            _onDrawEnd: function(graphic, geotype, commontype) {
				...
				if (this.measurePolylineGraphic) {
					this.measureGraphicsLayer.remove(this.measurePolylineGraphic);
				}

                this.measureGraphic = null;
                this.measureGraphic2 = null;
				this.measurePolylineGraphic = null;
            ...
            _measureMove: function(e) {
                ...
                if (gra) {
                    ...
                    // display an interactive measurement of the polyline when there are more than 2 segments
                    if (l > 1 && gra.geometry.type === 'polyline') {
						var path = [];
						for (var i = 0; i < gra.geometry.paths[0].length; i++) {
							path.push(gra.geometry.paths[0][i]);
						}
						path.push([e.mapPoint.x, e.mapPoint.y]);
						var currentPL = new Polyline(this.map.spatialReference);
						currentPL.addPath(path);
						var polyLength = this._calculatePolylineLength(currentPL);
						this._addPolylineTotalMeasure(polyLength, e.mapPoint);
                    }
            ...
            _addPolylineTotalMeasure: function(length, pt) {
                var a = Font.STYLE_ITALIC;
                var b = Font.VARIANT_NORMAL;
                var c = Font.WEIGHT_BOLD;
                var symbolFont = new Font(this.textSymChooser.textFontSize.value + "px", a, b, c, "Courier");
                var fontColor = this.textSymChooser.textColor.color;
                var textSymbol = new TextSymbol(length, symbolFont, fontColor);
				textSymbol.setOffset(60, 10);
				textSymbol.setAlign(Font.ALIGN_START);
                if (this.measurePolylineGraphic === null || this.measurePolylineGraphic._graphicsLayer === null) {
                    this.measurePolylineGraphic = new Graphic(pt, textSymbol, null, null);
                    this.measureGraphicsLayer.add(this.measurePolylineGraphic);
                } else {
                    this.measurePolylineGraphic.setGeometry(pt);
                    this.measurePolylineGraphic.setSymbol(textSymbol);
                }
            },
